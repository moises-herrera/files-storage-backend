...
export class Migration20250323150931 extends Migration {
  ...
  override async up(): Promise<void> {
    ...
    this.addSql(`create table "user" ("id" uuid not null, "created_at" timestamptz not null, "updated_at" timestamptz not null, "first_name" varchar(20) not null, "last_name" varchar(20) not null, "email" varchar(50) not null, "password" text not null, constraint "user_pkey" primary key ("id"));`);
    ...
    this.addSql(`create table "file" ("id" uuid not null, "created_at" timestamptz not null, "updated_at" timestamptz not null, "name" varchar(100) not null, "extension" varchar(100) not null, "size" int not null, "mime_type" varchar(100) not null, "storage_path" varchar(255) not null, "owner_id" uuid not null, "folder_id" uuid not null, constraint "file_pkey" primary key ("id"));`);
    ...
    this.addSql(`alter table "file" add constraint "file_owner_id_foreign" foreign key ("owner_id") references "user" ("id") on update cascade;`);
  ...
  override async down(): Promise<void> {
    ...
    this.addSql(`alter table "file" drop constraint "file_owner_id_foreign";`);
